<template>
  <div class="edit container">
    ...
    <h1 class="pb-2 mb-3 pt-3 border-bottom">Edit Customer</h1>
...
<script>
  ...
  export default {
    ...
    methods: {
      fetchCustomer() {
        this.$http.get('http://localhost/slimapp/public/api/customer/'+this.$route.params.id)
          .then(function(response) {
            this.customer = response.body;
          });
      },
      updateCustomer(e) {
        if(!this.customer.first_name || !this.customer.last_name || !this.customer.email || !this.customer.address) {
          this.alert = 'Please fill in all required fileds';
        } else {
          let updateCustomer = {
          ...
          this.$http.put('http://localhost/slimapp/public/api/customer/update/'+this.$route.params.id, updateCustomer)
            .then(function(response) {
